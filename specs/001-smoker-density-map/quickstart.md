...
8. **底部免責聲明 (FR-017)**：開啟任意頁面，確認頁面底部固定顯示包含「主觀眾包意見、非官方數據、平台不負確準責任」字樣的免責聲明。
9. **送出前確認彈窗 (FR-018)**：匿名使用者選定地點後點擊送出 → 確認彈窗出現，確認後才送出；取消則不呼叫 API。
10. **隱私政策頁 (FR-019)**：直接造訪 `/privacy`（或底部連結）→ 顯示包含「Cloudflare Turnstile 第三方資料處理者」說明及申訴信箱的靜態頁面。

## 預期結果

上述 1–10 全數通過即代表 MVP（US1）、完整回報循環（US2/US3）及法律合規（FR-017~019）端到端可運作。
